...
const Journal = () => {
  ...
  const handleEditFromCard = (entries: JournalEntry[]) => {
    // Find the full jurnal data from the entries
    const code = entries[0]?.code;
    const jurnal = jurnalData?.data?.find(j => j.id_ju === code);
    if (jurnal) {
      handleEdit(jurnal);
    }
  };
  ...
  const handleDeleteFromCard = (code: string) => {
    handleDelete(code);
  };
              ...
              {isLoading ? (
                ...
                filteredData.map((jurnal) => (
                  <JournalEntriesCard
                    key={jurnal.id_ju}
                    code={jurnal.id_ju}
                    entries={jurnal.jurnal?.map((entry: any) => ({
                      code: jurnal.id_ju,
                      date: jurnal.tanggal,
